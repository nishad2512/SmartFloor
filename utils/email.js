...
export const sendResetEmail = async (recipientEmail, url) => {
    const mailOptions = {
        from: `"SmartFloor Support" <${process.env.EMAIL_USER}>`,
        to: recipientEmail,
        subject: 'SmartFloor: Reset your password',
        html: `
            <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ccc; max-width: 600px; margin: auto;">
                <h2 style="color: #333;">Reset Your SmartFloor Account Password</h2>
                <p>Thank you for registering. Please use the following url to complete your reset:</p>
                <div style="background: #f8f8f8; padding: 15px; text-align: center; border-radius: 5px;">
                    <strong style="font-size: 24px; color: #136dec;"><a href="${url}">Click here</a></strong>
                </div>
                <p style="font-size: 12px; color: #999;">This url will expire in 5 minutes.</p>
            </div>
        `,
    };
    ...
    try {
        ...
        console.log(process.env.EMAIL_PASSWORD)
        const info = await transporter.sendMail(mailOptions);
        console.log('URL Email Sent: %s', info.messageId);
        return true;
    } catch (error) {
        console.error('Error sending OTP email:', error);
        return false;
